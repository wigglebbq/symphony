	...
	for _, entry := range o.running {
		running = append(running, map[string]any{
			...
			"workflow": map[string]any{
				...
				"review_mode":             entry.workflow.ReviewMode,
				...
				"review_summary":          entry.workflow.ReviewSummary,
				"reviewed_sha":            entry.workflow.ReviewedSHA,
				"review_artifact_path":    entry.workflow.ReviewArtifactPath,
				"last_sync_at":            formatTime(entry.workflow.LastSyncAt),
				"last_review_synced_at":   formatTime(entry.workflow.LastReviewSyncedAt),
